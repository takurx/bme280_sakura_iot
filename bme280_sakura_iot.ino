/*
   BME280 and Arduino Genuino Uno on Sakura IoT platform beta V0.0
   takuzirra@twitter
   mail: taku3wzr@gmail.com

   sakura iot beta module 
   use I2C1

   BEM280
   use I2C2
   
   Arduino Genuino Uno -  BME280
   3.3V                -  VDD 3.3V (P1)
   GND                 -  GND      (P2)
   NC                  -  CSB      (P3)
   SDA                 -  SDI      (P4)
   ...
   reference:
   1. http://trac.switch-science.com/wiki/BME280
   2. https://github.com/takurx/SakuraIOArduino
*/
...
void loop() {
  ...
  uint32_t temp_int = 0, press_int = 0,hum_int = 0;
  ...
  //temp_int = (uint32_t)(temp_act);
  //press_int = (uint32_t)(press_act);
  //hum_int = (uint32_t)(hum_act);
  uint64_t offset = 0;
  double test = 0.0;
  uint64_t test1 = 0;
  
  sakuraio.enqueueTx(0, (float)(temp_act));
  sakuraio.enqueueTx(1, (float)(press_act));
  sakuraio.enqueueTx(2, (float)(hum_act));
  sakuraio.send();
